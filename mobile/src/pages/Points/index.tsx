        ...
        <>
                        ...
                        <MapView style={styles.map} initialRegion={{
                            ...
                            {points.map(point => (
                                <Marker key={String(point.id)} coordinate={{
                                    latitude: point.latidude || 0,
                                    longitude: point.longitude || 0,
                                }} style={styles.mapMarker} onPress={() => handleNavigationToDetail(point.id)}>
            ...
            <View style={styles.itemsContainer}>
                <ScrollView contentContainerStyle={{paddingHorizontal: 20}} showsHorizontalScrollIndicator={false} horizontal>
                    {items.map(item => (
                        <TouchableOpacity activeOpacity={0.7} key={String(item.id)} style={[
                            styles.item,
                            selectedItems.includes(item.id) ? styles.selectedItem : {}
                        ]} onPress={() => handleSelectedItem(item.id)}>
                            <SvgUri uri={item.imgUrl} width={42} height={42}/>
